# LSP5004 V46 — Professional Fast AACR2 Catalogue Card Engine

V46 is the professional fast catalogue-card build for LSP5004.
...
The PDF filename and deployed server version are also V46.


## V46 stability/layout fix
- Each AACR2 description field starts in the same description column; it never inherits the previous field's start position.
- Wrapped lines stay within their field row; subordinate subject/tracing lines use a small subordinate indent.
- Horizontal ruling is aligned to the eight content rows instead of crossing catalogue text.
- Vertical ruling remains fixed at the filing/indent columns.
- Screen and PDF use the same validated `layoutLines` and the same bold Courier catalogue text.
- Serial parsing preserves labelled physical description, series, multi-line notes, subjects, and added-entry blocks.
- Package version markers are consistent across the deployment files.
