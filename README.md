# LSP5004 V44 — Professional Fast AACR2 Catalogue Card Engine

V44 is the professional fast catalogue-card build for LSP5004.
...
The PDF filename and deployed server version are also V44.


V44 card visibility/ruling fix: browser and PDF use the same 10-row 5 x 3 card grid, conservative line wrapping, bold readable Courier text, two vertical indentions and horizontal ruling. Gemini requests use Google Search grounding when a Gemini API key is configured; local AACR2 fallback remains automatic if Gemini/Search is unavailable.


Card-fit-only patch: preserved V44 ruling/indentation and improved text wrapping so physical card rows do not clip.
