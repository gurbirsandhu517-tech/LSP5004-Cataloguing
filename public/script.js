(() => {
  ...
  function localAnswer(q,material){
    ...
    if (/\bmain\s+entr(?:y|ies)\b/.test(ql)) requestedEntries.push({label:"Main Entry",value:title||"Not supplied"});
  ...
  function render(r){
    ...
    put("entry",entry);
    const wrap=$("questionEntries"), list=$("requestedEntriesList");
    if(wrap && list){ list.innerHTML=""; const items=Array.isArray(r.requestedEntries)?r.requestedEntries:[]; items.forEach(item=>{ const card=document.createElement("div"); card.className="requested-item"; const h=document.createElement("h4"); h.textContent=val(item.label); const pre=document.createElement("pre"); pre.textContent=val(item.value); card.append(h,pre); list.appendChild(card); }); wrap.classList.toggle("hidden",items.length===0); }
