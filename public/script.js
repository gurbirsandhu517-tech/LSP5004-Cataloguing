document.addEventListener('DOMContentLoaded', () => {
    ...
    if (generateBtn) {
        generateBtn.addEventListener('click', async () => {
            ...
            cardView.innerHTML = '<div style="padding: 8px; font-weight: bold; font-size: 12px;">Generating aligned catalogue cards...</div>';
            ...
            try {
                ...
                if (data.success && data.result && data.result.cards) {
                    ...
                    data.result.cards.forEach((card, index) => {
                        let rowsHtml = card.lines.map(line => `
                            <tr>
                                <td class="col-margin-1"></td>
                                <td class="col-margin-2"></td>
                                <td class="col-content">${line}</td>
                            </tr>
                        `).join('');
