document.addEventListener('DOMContentLoaded', () => {
    ...
    if (generateBtn) {
        generateBtn.addEventListener('click', async () => {
            ...
            cardView.innerHTML = '<div style="padding: 8px; font-weight: bold; font-size: 12px;">Generating universal AACR2 cards...</div>';
            ...
            try {
                ...
                if (data.success && data.result && data.result.cards) {
                    ...
                    data.result.cards.forEach((card, index) => {
                        let linesHtml = card.lines.map(line => `<div class="card-line-text">${line}</div>`).join('');
                        cardsHtml += `
