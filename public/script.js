...
function splitLong(lines,max=82){const out=[];for(const raw of lines||[]){let s=String(raw||"").trim();if(!s)continue;while(s.length>max){let cut=s.lastIndexOf(" ",max);if(cut<40)cut=max;out.push(s.slice(0,cut).trim());s=s.slice(cut).trim();}if(s)out.push(s);}return out;}
function cardHtml(c,i){const left=[c.callNumber,c.accessionNumber].filter(Boolean).map(esc).join("<br>");return `<article class="catalogCard"><div class="cardNo">CARD ${i+1}</div><div class="cardType">${esc(c.entryType||c.cardType||"CATALOGUE ENTRY")}</div><div class="cardLeft">${left}</div><div class="cardText">${splitLong(c.lines).map(esc).join("\n")}</div></article>`;}
