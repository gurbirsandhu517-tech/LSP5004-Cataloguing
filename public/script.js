...
function render(r){
 ...
 $("answerTitle").textContent=`${n} catalogue ${n===1?"card":"cards"} generated`;
 $("resultMeta").textContent=[r.materialType,r.provider].filter(Boolean).join(" · ");
 ...
 renderCards(r.cards); $("result").classList.remove("hidden");
 $("status").textContent=r.warning?`${r.provider||"AACR2 automatic catalogue engine"} — complete valid entry set prepared.\nGemini note: ${r.warning}`:`${r.provider||"AACR2 automatic catalogue engine"} — complete valid entry set prepared.`;
}
