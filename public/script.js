...
function renderCards(cards){$('cards').innerHTML=(cards||[]).map((c,i)=>{const lines=(c.layoutLines||((c.lines||[]).map((x,j)=>({text:x,kind:j?'continuation':'title'})))).map((x,j)=>{const kind=x.kind||'continuation';return `<span class="cardLine card-${kind}">${esc(x.text||'')}</span>`}).join('');return `<article class="catalogCard"><div class="cardNo">CARD ${i+1} / ${cards.length}</div><div class="cardType">${esc(c.entryType||'MAIN ENTRY')}</div><div class="cardLeft">${esc(c.callNumber||'')}</div><div class="cardText">${lines}</div></article>`}).join('')}
