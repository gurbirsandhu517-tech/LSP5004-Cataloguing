// V18 unlimited-entry engine: no legacy A/B/C/D section selector is required.
...
function cardHtml(c,i){const left=[c.callNumber,c.accessionNumber].filter(Boolean).map(esc).join("<br>");const type=esc(c.entryType||c.cardType||"CATALOGUE ENTRY");return `<article class="catalogCard"><div class="cardNo">CARD ${i+1}</div><div class="cardType">${type}</div><div class="cardLeft">${left}</div><div class="cardText">${(c.lines||[]).map(esc).join("\n")}</div></article>`;}
...
function askForGeminiKeyIfNeeded(){
  if(sessionStorage.getItem("lspGeminiKey")) return sessionStorage.getItem("lspGeminiKey");
  const k=prompt("Gemini API key is not configured on the server. Paste your Gemini API key for this session (it will not be saved permanently):");
  if(k&&k.trim()){sessionStorage.setItem("lspGeminiKey",k.trim());return k.trim();}
  return "";
}
$("generate").onclick=async()=>{const q=$("question").value.trim();if(!q){alert("Paste the complete cataloguing question first.");return;}const b=$("generate");b.disabled=true;b.textContent="GENERATING ALL ENTRIES…";$("loading").classList.remove("hidden");$("status").textContent="Checking Gemini and building every required entry…";try{let key=sessionStorage.getItem("lspGeminiKey")||"";try{const h=await fetch("/health",{cache:"no-store"}).then(r=>r.json());if(!h.geminiConfigured&&!key) key=askForGeminiKeyIfNeeded();}catch{}const res=await fetch("/generate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({question:q,section:"",materialType:selectedMaterial,apiKey:key})});const data=await res.json().catch(()=>({}));if(!res.ok)throw new Error(data.error||`Generation failed (${res.status}).`);render(data);if(data.error) $("status").textContent=(data.provider||"AACR2 engine")+" — "+data.error;}catch(e){$("status").textContent=e.message||"Generation failed.";alert($("status").textContent);}finally{b.disabled=false;b.textContent="⚡ GENERATE ALL CATALOGUE ENTRIES";$("loading").classList.add("hidden");}};
