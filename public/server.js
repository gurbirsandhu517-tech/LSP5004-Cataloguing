...
const VERSION = "V48-AACR2-TRADITIONAL-CARD-ACCURATE";
...
// Traditional catalogue-card layout based on the Type-II card model used in
// library-cataloguing instruction: a paragraph starts at the second indention;
// wrapped continuation lines return to the first indention. A new paragraph/area
// always starts at the paragraph margin. This is a layout rule, not a guess about
...
function wrapCardField(text,maxChars){
  ...
  for(const w of words){
    ...
    if(next.length>maxChars){ if(line) out.push(line); line=w; }
    else line=next;
  }
...
function prepareTraditionalCardLines(lines){
  ...
  for(const line of arr(lines)){
    const s=str(line).trim();
    if(!s) { out.push({text:'',kind:'blank'}); continue; }
    const classified = /^Tracing\s*:/i.test(s) ? splitTracingLine(s)
      : /^Subjects?\s*:/i.test(s) ? splitSubjectLine(s)
      : [classifyCardLine(s,out.length)];
    for(const item of classified){
      const text=str(item.text).trim(); if(!text) continue;
      const kind=item.kind||'field';
      // The paragraph's first physical line begins at the second indention.
      // Every wrapped continuation begins at the first indention.
      const wrapped=wrapCardField(text,46);
      wrapped.forEach((t,j)=>out.push({text:t,kind:j===0?kind:'continuation',paragraphStart:j===0}));
    }
  }
  return out;
}
...
function expandCards(cards){
  const out=[];
  // 10 generous writing rows keep the card readable and prevent ruling from
  // crossing glyphs. More content creates a genuine continuation card.
  const maxRows=10;
...
function drawCard(doc,card,no,total){
  ...
  // Two vertical indention/ruling lines. Text is in the description area;
  // paragraph first lines start at the second indention, continuations at the first.
  const v1=34,v2=58;
  const firstX=v2+6;
  const continuationX=v1+6;
  const right=W-7;
  const headerH=25;
  const rows=10;
  ...
  doc.save();
  doc.rect(0,0,W,H).lineWidth(0.9).strokeColor('#111').stroke();
  doc.lineWidth(0.6).strokeColor('#222');
  doc.moveTo(v1,0).lineTo(v1,H).stroke();
  doc.moveTo(v2,0).lineTo(v2,H).stroke();
  // Horizontal writing/ruling lines begin below the header and never cross text.
  ...
  doc.fillColor('#111').font('Helvetica-Bold').fontSize(6.8)
    .text(str(card.cardType||card.entryType||'CATALOGUE ENTRY').toUpperCase(),firstX,7,{width:W-firstX-72,align:'left',lineBreak:false});
  doc.fillColor('#555').font('Helvetica-Bold').fontSize(6.4)
    .text(`CARD ${no}${total?' / '+total:''}`,firstX,7,{width:W-firstX-5,align:'right',lineBreak:false});
  ...
  if(card.accessionNumber) doc.fillColor('#000').font('Courier-Bold').fontSize(6.2)
    .text(card.accessionNumber,3,headerH+rowH*5+1,{width:v1-6,align:'left',lineBreak:false});

  const fs=8.8;
  for(let i=0;i<Math.min(prepared.length,rows);i++){
    const item=prepared[i];
    const text=str(item.text); if(!text) continue;
    const isSub=item.kind==='sub';
    const x=isSub ? firstX+11 : (item.kind==='continuation' ? continuationX : firstX);
    const width=right-x;
    const y=headerH+i*rowH+(rowH-fs*1.15)/2-1;
