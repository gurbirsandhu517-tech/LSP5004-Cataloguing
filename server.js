...
const VERSION = "V42-AACR2-TRADITIONAL-CARD-LAYOUT";
...
function splitTracingLine(line){
  ...
  if(!m) return [str(line)];
  const items=m[1].split(/\s{2,}(?=\d+\.\s)|(?<=\.)\s+(?=\d+\.\s)/).map(x=>x.trim()).filter(Boolean);
  return ['Tracing:', ...items];
}
function splitSubjectLine(line){
  const m=str(line).match(/^Subjects?\s*:\s*(.+)$/i);
  if(!m) return [str(line)];
  const parts=m[1].split(/\s*;\s*/).map(x=>x.trim()).filter(Boolean);
  return parts.length>1 ? ['Subjects:', ...parts.map((x,i)=>`${i+1}. ${x}`)] : [str(line)];
}
function prepareTraditionalCardLines(lines){
  const expanded=[];
  for(const raw of arr(lines)){
    if(/^Tracing\s*:/i.test(str(raw))) expanded.push(...splitTracingLine(raw));
    else if(/^Subjects?\s*:/i.test(str(raw))) expanded.push(...splitSubjectLine(raw));
    else expanded.push(str(raw));
  }
  return expanded;
}
function fieldIndent(line,index){
  const s=str(line).trim();
  if(index===0) return 0;

  // A new AACR2 bibliographic element starts at the field's normal text
  // position. Only a wrapped continuation of that SAME element is indented.
  const newField=/^(?:Edition(?: statement)?|Place of publication|Publisher|Date|Frequency|Numbering|Vol\.|Physical description|Series|Notes?|ISSN|ISBN|Continues|Continued by|Former title|Includes|Subjects?|Subject coverage|Added entries|Added entry|Tracing|Call(?: No\.?| number)|Classification|Accession(?: no\.?| number))\s*:/i;
  if(newField.test(s)) return 0;
  if(/^\d+\.\s/.test(s)) return 4;

  // Title/responsibility and other unlabeled continuation lines use the
  // traditional hanging continuation indent.
  return 4;
}
function wrapForCard(text,maxChars=42,indent=0){
  const clean=str(text).trim(); if(!clean)return [];
  const prefix=' '.repeat(indent);
  const available=Math.max(12,maxChars-indent);
  const words=clean.split(/\s+/).filter(Boolean);
  const out=[]; let line='';
  for(const w of words){
    if(w.length>available){
      if(line){out.push(prefix+line);line='';}
      for(let i=0;i<w.length;i+=available) out.push(prefix+w.slice(i,i+available));
      continue;
    }
    const next=line?line+' '+w:w;
    if(next.length>available){out.push(prefix+line);line=w;}else line=next;
  }
...
function expandCards(cards){
  ...
  for(const card of arr(cards)){
    const sourceLines=prepareTraditionalCardLines(card.lines);
    const wrapped=[];
    sourceLines.forEach((line,index)=>{
      const indent=fieldIndent(line,index);
      wrapped.push(...wrapForCard(line,42,indent));
    });
...
app.post("/generate-pdf",(req,res)=>{
  ...
  res.setHeader("Content-Disposition",'attachment; filename="LSP5004-AACR2-Catalogue-Cards-V42.pdf"');
  doc.pipe(res);
