...
const VERSION = "V39-AACR2-PROFESSIONAL-FAST";
...
function expandCards(cards){
  ...
  for(const card of arr(cards)){
    const wrapped=[];for(const line of arr(card.lines))wrapped.push(...wrapForCard(line,42));
...
app.post("/generate-pdf",(req,res)=>{
  ...
  res.setHeader("Content-Disposition",'attachment; filename="LSP5004-AACR2-Catalogue-Cards-V39.pdf"');
  doc.pipe(res);
