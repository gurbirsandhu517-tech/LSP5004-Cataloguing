...
const VERSION = "V28-AACR2-NATURAL-QUESTION-FIX";
...
function parseSerialQuestion(q, materialType){
  ...
  if(!titleClause){
    const nf=inferNaturalFields(q,'Serial Publication');
    if(nf.title) titleClause=nf.title+(nf.editor?' / edited by '+nf.editor:'');
  }
  ...
  if(!titleStatement){
    const nf=inferNaturalFields(q,'Serial Publication');
    titleStatement=nf.title;
    if(!responsibility && nf.editor) responsibility='edited by '+nf.editor;
  }
  ...
  const frequency=[frequencyClause,naturalFreq,frequencyChange && frequencyChange!==frequencyClause?frequencyChange:''].filter(Boolean).filter((v,i,a)=>a.indexOf(v)===i).join(' ');
  ...
  const natural=inferNaturalFields(q,'Serial Publication');
  const issn=strip(findClause(/^ISSN\s+/i)).replace(/^ISSN\s+/i,'') || natural.issn;
  const naturalFreq=natural.frequency;
  const naturalImprint=(imprint || [natural.place,natural.publisher,natural.year].filter(Boolean).join(' : '));
  const naturalPhysical=(physical || natural.format);
  ...
  if(naturalImprint) main.push(naturalImprint.replace(/[.]?$/,'') + '.');
  if(naturalPhysical) main.push(naturalPhysical.replace(/[.]?$/,'') + '.');
...
app.post("/generate-pdf",(req,res)=>{
  ...
  res.setHeader("Content-Disposition",'attachment; filename="LSP5004-AACR2-Catalogue-Cards-V28.pdf"');
  doc.pipe(res);
