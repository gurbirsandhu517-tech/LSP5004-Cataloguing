...
const VERSION = "V32-AACR2-FULL-REBUILD";
...
function localParse(question, section, materialType){
  ...
  if(inferredType==='All Materials') {
    const hasSerialClue=/ISSN|serial publication|continuing resource|periodical|quarterly|monthly|bimonthly|biweekly|weekly|semiannual|annual|Vol\.\s*\d+\s*,\s*no\.\s*\d+/i.test(q);
    const hasJournalContext=/journal/i.test(q) && /(?:published|issued|periodical|volume|issue|ISSN|quarterly|monthly|bimonthly|weekly|annual)/i.test(q);
    ...
    else if(/book|monograph|textbook|author|ISBN/i.test(q)) inferredType='Book';
...
function normalizeResult(raw, question, section, materialType){
  ...
  const looksPlaceholder = /^(?:this item|this work|this resource|the item|the work|untitled|complete valid entry set prepared)/i.test(title);
  if(!title || looksPlaceholder){
...
app.post("/generate",async(req,res)=>{
  ...
  try{
    ...
    try {
      const emergency = parseSerialQuestionRobust(q, materialType);
...
app.post("/generate-pdf",(req,res)=>{
  ...
  res.setHeader("Content-Disposition",'attachment; filename="LSP5004-AACR2-Catalogue-Cards-V31.pdf"');
  doc.pipe(res);
