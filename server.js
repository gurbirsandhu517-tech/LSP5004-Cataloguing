...
const VERSION = "V20-AACR2-ADVANCED-CARD-ENGINE";
...
function parseSerialQuestion(q, materialType){
  ...
  // Extract names only from the responsibility statement. Handle comma + "and" lists
  // without confusing an inverted name such as "Sharma, R. K.".
  const names=[];
  const respMatch=responsibility.match(/^(?:edited by|compiled by|prepared by|by)\s+(.+)$/i);
  if(respMatch){
    let list=respMatch[1].replace(/[.]$/,'').replace(/,\s+and\s+/ig,' and ');
    for(const part of list.split(/\s+and\s+/i)){
      const bits=part.split(/,\s*(?=[A-Z][A-Za-z.\-]*(?:\s|$))/).map(strip).filter(Boolean);
      if(bits.length>1 && !/^.+,\s*(?:[A-Z]\.?\s*){1,4}$/.test(part.trim())) names.push(...bits);
      else if(part.trim()) names.push(part.trim());
    }
  }
  // De-duplicate names while preserving their supplied order.
  const uniqueNames=[]; const seenNames=new Set();
  for(const n of names){const k=n.toLowerCase(); if(!seenNames.has(k)){seenNames.add(k);uniqueNames.push(n);}}
  names.length=0; names.push(...uniqueNames);
...
function localParse(question, section, materialType){
  ...
  const addedEntries=[];
  // Deterministic fallback: only create access points that the supplied statement supports.
  if(resp){
    const raw=resp.replace(/^(?:by|written by|authored by|edited by|compiled by)\s+/i,'').replace(/[.]$/,'');
    const names=raw.replace(/,\s+and\s+/ig,' and ').split(/\s+and\s+/i).flatMap(part=>part.split(/,\s*(?=[A-Z][A-Za-z.\-]*(?:\s|$))/)).map(x=>x.trim()).filter(Boolean);
    const unique=[...new Map(names.map(n=>[n.toLowerCase(),n])).values()];
    unique.forEach(n=>{
      let h=n; if(!/,\s*[A-Z][A-Za-z.\-]+/.test(n) && n.split(/\s+/).length>1){const b=n.split(/\s+/);h=b.pop()+', '+b.join(' ');}
      addedEntries.push({type:'personal name',heading:h.replace(/[.]$/,'')+'.',reference:'Personal-name added entry supported by the responsibility statement.'});
    });
  }
  if(series) addedEntries.push({type:'series',heading:series.replace(/[.]$/,'')+'.',reference:'Series added entry supported by the supplied series statement.'});
  const tracing=addedEntries.map((a,i)=>`${i+1}. ${a.heading}`).join('  ');
  if(tracing) lines.push('Tracing: '+tracing);
  const cards=[{entryType:"MAIN ENTRY",cardType:"Main Entry",callNumber:call,accessionNumber:accession,lines}];
  const body=lines.filter(x=>!/^Tracing\s*:/i.test(x));
  for(const a of addedEntries) cards.push({entryType:a.type.includes('series')?'ADDED ENTRY — SERIES':'ADDED ENTRY — PERSONAL NAME',cardType:'Added Entry',callNumber:call,accessionNumber:'',lines:[a.heading,...body]});
  if(title && !/^(?:title entry|main entry)$/i.test(resp)){}
  return {materialType:inferredMaterial,mainEntry:title||resp||"",titleStatement:title,responsibility:resp,edition,imprint,physicalDescription:physical,notes,series,seriesEntry:series,callNumber:call,accessionNumber:accession,addedEntries,subjectEntries:[],cards,verification:"Structured AACR2 fallback used because Gemini could not be reached. No unsupported bibliographic facts were invented.",missing:["Gemini generation/verification was unavailable; review this AACR2 practice answer before submission."]};
}
...
function wrapForCard(text,maxChars=58){
...
function expandCards(cards){
  ...
  for(const card of arr(cards)){
    const wrapped=[];for(const line of arr(card.lines))wrapped.push(...wrapForCard(line,58));
...
function drawCard(doc,card,no,total){
  ...
  doc.font('Helvetica-Bold').fontSize(5.2).fillColor('#555').text(`CARD ${no}${total?' / '+total:''}`,textX,4,{width:W-textX-4,align:'right'});
  if(card.callNumber) doc.font('Helvetica-Bold').fontSize(5.6).fillColor('#000').text(card.callNumber,3,7,{width:v1-6,align:'left'});
  // Practice convention requested by the user: accession number on the 5th line.
  if(card.accessionNumber) doc.font('Helvetica-Bold').fontSize(5.4).text(card.accessionNumber,3,126,{width:v1-6,align:'left'});
  doc.fillColor('#000').font('Courier-Bold').fontSize(7.25);
  let y=29;
  arr(card.lines).forEach((line,idx)=>{
    const t=str(line);if(!t)return;
    const x=idx===0?v1+7:v2+7;
    const h=doc.heightOfString(t,{width:W-x-7,lineGap:0.4});
    if(y+h>H-4)return;
    doc.text(t,x,y,{width:W-x-7,lineGap:0.4});
    y+=Math.max(8.5,h+1.2);
  });
}
