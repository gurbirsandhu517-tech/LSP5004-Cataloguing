...
const VERSION = "V23-AACR2-UNIVERSAL-ENTRY-ENGINE";
...
function localParse(question, section, materialType){
  ...
  if(inferredType==='All Materials') {
    if(/motion picture|film|feature film|cinema/i.test(q)) inferredType='Motion Picture';
    ...
    else if(/book|monograph|textbook/i.test(q)) inferredType='Book';
  }

  if(looksLikeInstructionOnly(q) || !title || /^(?:create|make|prepare|write|produce|generate)\b/i.test(title)) {
    const cleanType=inferredType==='All Materials'?'Not specified':inferredType;
    return {
      materialType:cleanType, mainEntry:'', titleStatement:'', responsibility:'', edition:'', materialSpecific:'', imprint:'', physicalDescription:'', series:'', notes:'', standardNumber:'', callNumber:call, accessionNumber:accession,
      addedEntries:[], subjectEntries:[], cards:[],
      verification:'No catalogue card was fabricated because the request contains instructions but no identifiable bibliographic title/details.',
      missing:[cleanType==='Motion Picture' ? 'Motion picture title and bibliographic details are required: title, responsibility statement, publication/distribution details, date, and physical description where available.' : 'A bibliographic title and the relevant descriptive details are required.']
    };
  }
  ...
  const added=[];
  if(resp){
    const m=resp.match(/^(?:edited by|compiled by|prepared by|written by|directed by|produced by|by)\s+(.+)$/i);
    if(m){
      ...
      for(const part of parts){
        ...
        const role=/directed by/i.test(resp)?'director':/produced by/i.test(resp)?'producer':/edited by|compiled by|prepared by/i.test(resp)?'editor':'author';
        h=h.replace(/,\s*(?:director|producer|editor|author|ed\.)$/i,'')+', '+role+'.';
        added.push({type:'personal name',heading:h,reference:'Personal-name added entry supported by the responsibility statement.'});
        cards.push({entryType:'ADDED ENTRY — PERSONAL NAME',cardType:'Added Entry',callNumber:call,accessionNumber:'',lines:[h,title+'.']});
      }
    }
  }
  return {
    materialType:inferredType==='All Materials'?'Not specified':inferredType, mainEntry:title, titleStatement:title, responsibility:resp, edition:'', imprint:'', physicalDescription:'', series:'', notes:'', standardNumber:'', callNumber:call, accessionNumber:accession,
    addedEntries:added, subjectEntries:[], cards,
    verification:'Structured AACR2 fallback used from the supplied bibliographic/title evidence. No unsupported facts were invented.',
    missing:['Full bibliographic details were not supplied; review missing AACR2 areas before final submission.']
  };
}
...
function normalizeResult(raw, question, section, materialType){
  ...
  // Never let a failed/empty model response erase an explicitly selected material.
  if(materialType && materialType!=='All Materials' && (!r.materialType || /^not specified$/i.test(r.materialType))) r.materialType=materialType;
  ...
  const promptLeak=r.cards.some(c=>c.lines.some(x=>/catalogue the following|create a complete AACR2 catalogue entry|make all entries|complete cataloguing question|please make all entries/i.test(x)));
  ...
  if(promptLeak || looksLikeInstructionOnly(q) || !r.cards.some(c=>/^MAIN ENTRY$/i.test(c.entryType))){
    const local=localParse(q,section,materialType);
    r.cards=local.cards; r.mainEntry=local.mainEntry; r.titleStatement=local.titleStatement; r.responsibility=local.responsibility; r.imprint=local.imprint; r.physicalDescription=local.physicalDescription; r.notes=local.notes; r.series=local.series; r.callNumber=local.callNumber; r.accessionNumber=local.accessionNumber; r.materialType=local.materialType; r.addedEntries=local.addedEntries; r.subjectEntries=local.subjectEntries; r.missing=local.missing; r.verification=local.verification;
  }
...
async function geminiRequest(question, section, materialType, useSearch=true, apiKey=""){
  ...
  // V22: discover currently available Gemini generation models first. This avoids
  // hard-failing on retired model ids such as older *-flash-lite releases.
  const configured=[process.env.GEMINI_MODEL,"gemini-3.8-flash","gemini-3.7-flash","gemini-3.6-flash","gemini-3.5-flash","gemini-3.5-flash-lite","gemini-3.1-flash-lite"].filter(Boolean);
  const discovered=await getGeminiModels(key);
  const ranked=[
    ...configured.filter(m=>discovered.includes(m)),
    ...discovered.filter(m=>/flash/i.test(m) && !/image|tts|audio|embedding|live/i.test(m)),
    ...configured.filter(m=>discovered.length===0)
  ];
  const models=[...new Set(ranked)].slice(0,8); let last="";
...
app.post("/generate",async(req,res)=>{
  ...
  fallback.warning="";
  fallback.verification=(fallback.verification||"")+" AACR2 structured fallback was used automatically because the primary generation service was unavailable.";
...
function drawCard(doc,card,no,total){
  ...
  const v1=34,v2=58,textX=v1+7, textRight=W-7, textW=textRight-textX;
  const rowH=25; // PDF points; closely matches the on-screen catalogue-card ruling.

  doc.save();
  doc.rect(0,0,W,H).lineWidth(0.8).strokeColor('#111').stroke();
  doc.lineWidth(0.55).strokeColor('#555');
  ...
  doc.fillColor('#222').font('Helvetica-Bold').fontSize(6.2)
    .text(`CARD ${no}${total?' / '+total:''}`,textX,4,{width:W-textX-4,align:'right',lineBreak:false});
  if(card.callNumber){
    doc.fillColor('#000').font('Helvetica-Bold').fontSize(6.2)
      .text(card.callNumber,3,6,{width:v1-6,align:'left',lineBreak:false});
  }
  if(card.accessionNumber){
    doc.fillColor('#000').font('Helvetica-Bold').fontSize(6.0)
      .text(card.accessionNumber,3,rowH*5+1,{width:v1-6,align:'left',lineBreak:false});
  }

  // Cards are already wrapped by expandCards(). Fit every line to the available
  // width instead of letting PDFKit push words outside the ruling.
  const lines=arr(card.lines);
  let y=29;
  for(const raw of lines){
    const text=str(raw); if(!text) continue;
    let fs=7.6;
    while(fs>5.0 && doc.widthOfString(text,{font:'Helvetica-Bold',size:fs})>textW) fs-=0.2;
    const lineHeight=Math.min(13, Math.max(8.5,fs+2.2));
    if(y+lineHeight>H-5) break;
    doc.fillColor('#000').font('Helvetica-Bold').fontSize(fs)
      .text(text,textX,y,{width:textW,height:lineHeight,ellipsis:false,lineBreak:false,continued:false});
    y+=lineHeight+1.1;
  }
  doc.restore();
}
...
app.post("/generate-pdf",(req,res)=>{
  ...
  res.setHeader("Content-Disposition",'attachment; filename="LSP5004-AACR2-Catalogue-Cards-V23.pdf"');
  doc.pipe(res);
