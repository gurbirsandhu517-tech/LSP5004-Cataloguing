...
// Serve the actual public folder.
app.use(express.static(publicPath, { index: false }));

// Root route: this prevents Render from returning a 404 at "/".
app.get("/", (req, res) => {
...
Respect the selected section and material type.
Include:
- material type
- title proper
...
function normalizeResult(result, section, provider) {
  ...
  for (const k of keys) out[k] =
    result?.[k] == null || String(result[k]).trim() === "" ? "Not supplied" : String(result[k]).trim();
  out.section = section;
...
async function handleGenerate(req, res) {
  ...
  // Gemini first; Groq automatically takes over if Gemini fails/quota is unavailable.
  try {
    return res.json(await generateWithGemini(q, section, materialType));
  } catch (error) {
  ...
  try {
    const result = await generateWithGroq(q, section, materialType);
    result.verification =
      `${result.verification} Provider used: Groq fallback because Gemini was unavailable or failed.`;
  ...
  return res.status(503).json({
    error: "Both AI providers failed. Check GEMINI_API_KEY and GROQ_API_KEY in Render → Environment, then redeploy.",
    details: errors
  });
}

// Keep both paths so an older frontend/API call also works.
...
function makePdf(req, res) {
  try {
    ...
    const doc = new PDFDocument({
      size: "A4",
      margin: 48,
      info: { Title: "LSP5004 Catalogue Entry", Author: "LSP5004 Cataloguing Practice" }
    });
    ...
    doc.font("Helvetica-Bold").fontSize(15)
      .text("LSP5004: KNOWLEDGE ORGANISATION & ADVANCED LIBRARY CATALOGUING", { align: "center" });
    doc.moveDown(0.3);
    doc.font("Helvetica").fontSize(10).text("(PRACTICE CATALOGUE ENTRY)", { align: "center" });
    doc.moveDown();
    ...
    doc.moveDown(0.35);
    doc.font("Helvetica").fontSize(11).text(pdfSafe(entry || "No entry generated."), { lineGap: 3 });
    doc.moveDown();
    ...
    for (const [label, value] of fields) {
      ...
      doc.font("Helvetica").fontSize(10).text(pdfSafe(value || "Not supplied"), { lineGap: 2 });
      doc.moveDown(0.45);
    }

    doc.fontSize(8).fillColor("#666666")
      .text("Practice output only. Verify against the authorised course/reference material.", { align: "center" });
    doc.end();
  } catch (error) {
...
app.post("/generate-pdf", makePdf);
app.post("/api/generate-pdf", makePdf);

// Helpful diagnostic instead of an unexplained 404.
app.use((req, res) => {
  res.status(404).send("Route not found. The LSP5004 server is running, but this URL does not exist.");
});
