...
const VERSION = "V46-AACR2-CATALOGUE-CARD-RULING-STABLE";
...
function drawCard(doc,card,no,total){
  ...
  const headerH=22, rows=18, rowH=(H-headerH)/rows;
  ...
  doc.fillColor('#111').font('Helvetica-Bold').fontSize(6.6)
    .text(str(card.cardType||card.entryType||'CATALOGUE ENTRY').toUpperCase(),descriptionX,7,{width:W-descriptionX-72,align:'left',lineBreak:false});
  doc.fillColor('#555').font('Helvetica-Bold').fontSize(6.2)
    .text(`CARD ${no}${total?' / '+total:''}`,descriptionX,7,{width:W-descriptionX-5,align:'right',lineBreak:false});
  ...
  for(let i=0;i<Math.min(prepared.length,rows);i++){
    ...
    const y=headerH+i*rowH+2.5;
    const x=(item.kind==='sub') ? descriptionX+12 : descriptionX;
    const width=right-x;
    doc.fillColor('#000').font('Courier-Bold').fontSize(fs)
      .text(text,x,y,{width,height:rowH-3,ellipsis:false,lineBreak:false,continued:false});
  }
  doc.restore();
}
