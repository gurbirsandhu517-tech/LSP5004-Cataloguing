...
const VERSION = "V25-AACR2-CARD-ACCURACY-FIX";
...
function titleFromQuestion(q){
  const labeled=extractLabeled(q,['Title','Title proper','Title of the work','Film title','Book title']);
  if(labeled) return labeled;
  const cleaned=stripInstructionPrefix(q);
  const slash=cleaned.match(/^(.*?)\s*\/\s*(.+?)(?:\s*[—–].*)?$/);
...
function normalizeResult(raw, question, section, materialType){
  ...
  const placeholderLeak=r.cards.some(c=>c.lines.some(x=>/^(?:this item|this work|this resource|the item|the work|the resource|untitled)\.?$/i.test(str(x).trim())));
  const mainHasRealContent=r.cards.some(c=>/^MAIN ENTRY$/i.test(c.entryType) && c.lines.some(x=>{const z=str(x).trim();return z && !/^(?:this item|this work|this resource|the item|the work|the resource|untitled)\.?$/i.test(z) && !/^(?:create|make|prepare|write|produce|generate)\b/i.test(z);}));
  ...
  const suppliedFacts={title:titleFromQuestion(q),director:extractLabeled(q,['Director','Directed by']),author:extractLabeled(q,['Author','Creator','Writer']),screenplay:extractLabeled(q,['Screenplay','Screenwriter']),producer:extractLabeled(q,['Producer','Produced by']),publisher:extractLabeled(q,['Publisher','Production','Production/Distributor','Distributor','Producer/Distributor']),place:extractLabeled(q,['Place of Publication/Distribution','Place of Publication','Place of Distribution','Place','Publication Place','Distribution Place']),year:extractLabeled(q,['Year','Date','Date of Publication','Date of Distribution','Publication Date','Release Year']),running:extractLabeled(q,['Running Time','Duration','Running time'])};
  const mainText=arr(r.cards.find(c=>/^MAIN ENTRY$/i.test(c.entryType))?.lines).join(' ');
  const missingSupplied=Object.entries(suppliedFacts).filter(([k,v])=>v && !mainText.toLowerCase().includes(str(v).toLowerCase())).map(([k])=>k);
  if(serialLike){
  ...
  if(promptLeak || placeholderLeak || !mainHasRealContent || missingSupplied.length || looksLikeInstructionOnly(q) || !r.cards.some(c=>/^MAIN ENTRY$/i.test(c.entryType))){
...
async function geminiRequest(question, section, materialType, useSearch=true, apiKey=""){
  ...
  // V24: discover currently available Gemini generation models first. This avoids
...
function drawCard(doc,card,no,total){
  ...
  // The server has already wrapped the validated entry into card-sized lines.
  // Draw one physical catalogue line at a time; never allow PDFKit to reflow it.
  const lines=arr(card.lines).map(str).filter(Boolean);
  const availableH=H-headerH-7;
  const lineH=Math.min(12.5, Math.max(9.5, availableH/Math.max(lines.length,1)));
  let y=headerH+4;
  for(const raw of lines){
    let fs=7.1;
    while(fs>5.6 && doc.widthOfString(raw,{font:'Courier-Bold',size:fs})>textW) fs-=0.15;
    doc.fillColor('#000').font('Courier-Bold').fontSize(fs)
      .text(raw,textX,y,{width:textW,height:lineH,ellipsis:false,lineBreak:false,continued:false});
    y+=lineH;
    if(y>H-4) break;
  }
  doc.restore();
...
app.post("/generate-pdf",(req,res)=>{
  ...
  res.setHeader("Content-Disposition",'attachment; filename="LSP5004-AACR2-Catalogue-Cards-V25.pdf"');
  doc.pipe(res);
