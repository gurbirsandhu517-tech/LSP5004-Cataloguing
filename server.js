...
const VERSION = "V22-AACR2-UNIVERSAL-ENTRY-ENGINE";
...
app.post("/generate",async(req,res)=>{
  ...
  fallback.warning=lastError||"Gemini is not configured; structured AACR2 generation was used.";
  fallback.verification=(fallback.verification||"")+" "+(lastError?`Gemini retry note: ${lastError}`:"Gemini was not configured; structured AACR2 generation was used.");
...
function drawCard(doc,card,no,total){
  ...
  const lines=arr(card.lines).slice(0,10);
  let y=29;
...
app.post("/generate-pdf",(req,res)=>{
  ...
  res.setHeader("Content-Disposition",'attachment; filename="LSP5004-AACR2-Catalogue-Cards-V22.pdf"');
  doc.pipe(res);
