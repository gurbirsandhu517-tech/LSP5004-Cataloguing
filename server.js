...
Your job is to answer the COMPLETE assignment question, regardless of how the question is written: paragraph, bullets, numbered steps, table-like text, labels, or mixed wording.

SOURCE RULES:
- Treat the complete question as the source of bibliographic facts.
- Do not invent names, dates, places, publishers, dimensions, series, subjects, call numbers, or other facts that are not supplied or unambiguously derivable by an AACR2 rule from supplied facts.
- If a requested fact cannot be established from the supplied question, write "Not supplied" rather than guessing.
- Preserve supplied wording where AACR2 requires transcription (especially title proper and statement of responsibility).
- Use AACR2 terminology and punctuation appropriate to the material type selected.

QUESTION-AWARE RULES:
- First identify EVERY explicit task/request in the question (for example: main entry, main entry heading, title entry, subject entry/heading, added entry, author entry, corporate author entry, series entry, tracing, call number, physical description, notes, catalogue card, bibliographic description, or any other requested output).
- Do not assume a fixed template. If the question asks for 2 things, answer those 2; if it asks for 10 things, answer all 10.
- Keep requested outputs in the same order as they appear in the question.
- If the question asks to prepare a catalogue card/entry, make the complete catalogue-card text the main entry output and also provide each separately requested entry below it.
- If the question contains multiple records/items/titles, treat each as a separate item and answer each one. Do not merge them.
- If the question contains multiple subquestions (a), (b), (c), etc., create one requested-entry item for each subquestion and preserve the order.
- If the question asks for explanation, give a concise rule-based explanation using only supplied facts and standard AACR2 procedure; clearly distinguish anything that requires verification.
...
Each object represents one requested output/task found in the question, in the question's original order. Labels may be natural, e.g. "(a) Main Entry", "Subject Entry", "Added Entry", "Series Entry", "Call Number", "Tracing", "Catalogue Card", or another label matching the question.
If the question asks for multiple records/items, include all requested outputs for all items, clearly naming each item.
The entry field must contain the principal complete catalogue answer when a catalogue entry/card is requested; otherwise it should contain the most complete direct answer to the question.
Do not add unrelated sections or fixed fields merely because they exist in this JSON schema.`;
...
function normalise(r, section, materialType, provider) {
  ...
  const parsedRequested = requestedEntryItems(r?.__question || "", r);
  if (parsedRequested.length) requestedEntries = parsedRequested;
  return {
