...
function normalise(r, section, materialType, provider) {
  return {
    section: safe(r?.section, section || "A"),
    materialType: safe(r?.materialType, materialType || "Serial Publication"),
    title: safe(r?.title),
    responsibility: safe(r?.responsibility),
    edition: safe(r?.edition),
    publication: safe(r?.publication),
    physical: safe(r?.physical),
    notes: safe(r?.notes),
    subjectHeadings: safe(r?.subjectHeadings),
    accessPoints: safe(r?.accessPoints),
    missing: safe(r?.missing, "None reported"),
    verification: safe(r?.verification, "Check against authorised AACR2/course reference."),
    entry: safe(r?.entry, "No catalogue entry generated."),
...
app.get("/version", (req, res) => res.json({ ok: true, version: "GENERATE-V2-20260923" }));
