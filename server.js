...
function parseSerialQuestion(q, materialType){
  ...
  // Natural-language assignment questions often put the real serial title on
  // the same line as the instruction. Extract the quoted/title portion before
  // the responsibility slash so the instruction itself can NEVER become the title.
  if(!titleStatement){
    const sourceLine=raw.split(/\r?\n/).map(x=>x.trim()).find(line=>/\/\s*(?:edited|compiled|prepared|written|by)\b/i.test(line));
    if(sourceLine){
      const quoted=sourceLine.match(/[“\"]([^”\"]+)[”\"]\s*\/\s*(.+)$/i);
      if(quoted){
        titleStatement=strip(quoted[1]);
        responsibility=responsibility||strip(quoted[2]);
      }else{
        const plain=sourceLine.match(/(?:for|of)\s+(?:the\s+)?(?:serial publication|serial|periodical|journal)\s+(.+?)\s*\/\s*(.+)$/i);
        if(plain){
          titleStatement=strip(plain[1]);
          responsibility=responsibility||strip(plain[2]);
        }
      }
    }
  }
...
function prepareTraditionalCardLines(lines){
  ...
  for(let i=0;i<raw.length;i++){
    ...
    const wrapped=wrapCardField(item.text,34);
...
// SMART ACCURACY LAYER (local, deterministic):
// Gemini/Google Search can research, but this gate decides what is safe to print.
// It locks bibliographic facts to the user's question, detects prompt leakage,
// checks identifiers/names, removes duplicate access points, and rejects a
// generated record when the evidence is internally inconsistent.
function normalizeFactText(x){
  return str(x).toLowerCase().replace(/[“”"'`]/g,'').replace(/[^\p{L}\p{N}]+/gu,' ').replace(/\s+/g,' ').trim();
}
function extractEvidenceLedger(question){
  const q=str(question), compact=q.replace(/\r/g,' ').replace(/\n+/g,' ').replace(/\s+/g,' ');
  const ledger={titles:[],names:[],issns:[],isbns:[],years:[],places:[],publishers:[],udcs:[],callNumbers:[]};
  const add=(key,v)=>{v=str(v); if(v && !ledger[key].some(x=>normalizeFactText(x)===normalizeFactText(v))) ledger[key].push(v);};
  for(const m of q.matchAll(/[“\"]([^”\"]{3,180})[”\"]/g)) add('titles',m[1]);
  for(const m of q.matchAll(/\bISSN\s*[:#-]?\s*([0-9]{4}[-\s]?[0-9]{3}[0-9Xx])\b/gi)) add('issns',m[1]);
  for(const m of q.matchAll(/\bISBN\s*[:#-]?\s*([0-9Xx][0-9Xx\-\s]{8,20})\b/gi)) add('isbns',m[1].trim());
  for(const m of q.matchAll(/\b(?:19|20)\d{2}\b/g)) add('years',m[0]);
  for(const m of q.matchAll(/\b(?:edited by|compiled by|prepared by|written by|director|screenplay by|producer|author)\s+([^.;\n]+?)(?=\s*[.;]|\s+published by|\s+Third edition|\s+Second edition|\s+New Delhi|\s+Amritsar|\s+Chandigarh|\s*$)/gi)){
    m[1].split(/\s*;\s*|\s+and\s+/i).forEach(x=>add('names',x.replace(/^the\s+/i,'')));
  }
  const labels=[['places',['Place','Place of Publication','Publication Place']],['publishers',['Publisher','Publisher/Distributor']]];
  for(const [key,labels] of labels){ for(const lab of labels){ const re=new RegExp('^'+lab.replace(/[.*+?^${}()|[\\]\\]/g,'\\$&')+'\\s*[:\\-]\\s*(.+)$','im'); const m=q.match(re); if(m) add(key,m[1].split(/\n/)[0]); }}
  for(const m of q.matchAll(/\bUDC\s*[:#-]?\s*([^\s]+(?:\([^)]*\))?)/gi)) add('udcs',m[1]);
  for(const m of q.matchAll(/\bCall\s*(?:No\.?|number)\s*[:#-]?\s*([^\n]+)/gi)) add('callNumbers',m[1].trim());
  return ledger;
}
function factAppears(value, question){
  const v=normalizeFactText(value), q=normalizeFactText(question);
  if(!v || v.length<2) return true;
  if(q.includes(v)) return true;
  const words=v.split(' ').filter(w=>w.length>2);
  if(words.length<2) return q.includes(v);
  return words.filter(w=>q.includes(w)).length >= Math.max(2,Math.ceil(words.length*.65));
}
function collectGeneratedNames(r){
  const out=[];
  for(const a of arr(r.addedEntries)){ if(a?.heading) out.push(str(a.heading)); }
  for(const c of arr(r.cards)) for(const line of arr(c.lines)){
    const m=str(line).match(/^(?:I+|[A-Z])\.\s+(.+?)(?:,\s*(?:editor|author|director|producer|screenwriter|translator))?\.?$/i);
    if(m) out.push(str(m[1]));
  }
  return [...new Set(out.map(normalizeFactText).filter(Boolean))];
}
function dedupeAccessPoints(r){
  const seen=new Set(), out=[];
  for(const a of arr(r.addedEntries)){
    const key=normalizeFactText(a?.type+' '+a?.heading);
    if(!key || seen.has(key)) continue; seen.add(key); out.push(a);
  }
  r.addedEntries=out;
  return r;
}
function smartAccuracyGate(raw, question, materialType){
  const r=dedupeAccessPoints(raw && typeof raw==='object'?raw:{});
  const q=str(question), ledger=extractEvidenceLedger(q), issues=[];
  const allCards=arr(r.cards);
  const allLines=allCards.flatMap(c=>arr(c.lines).map(str)).filter(Boolean);
  const first=allLines[0]||str(r.titleStatement)||str(r.mainEntry);
  if(/^(?:please\s+)?(?:create|make|prepare|write|produce|generate)\b/i.test(first)) issues.push('Prompt leakage: instruction text appeared as the catalogue title.');
  if(!titleIsGroundedInQuestion(str(r.titleStatement||r.mainEntry||first),q)) issues.push('Title is not sufficiently grounded in the supplied question.');
  const joined=normalizeFactText(allLines.join(' '));
  for(const issn of ledger.issns){ if(!joined.includes(normalizeFactText(issn))) issues.push('ISSN mismatch.'); }
  for(const isbn of ledger.isbns){ if(/ISBN/i.test(joined) && !joined.includes(normalizeFactText(isbn))) issues.push('ISBN mismatch.'); }
  const names=collectGeneratedNames(r);
  for(const n of names){
    // Access-point headings must be traceable to the supplied question.
    if(n.length>3 && !factAppears(n,q)) { issues.push('Unsupported access point: '+n); break; }
  }
  // Duplicate physical lines are a common source of broken cards.
  const seenLines=new Set();
  for(const line of allLines){ const k=normalizeFactText(line); if(!k) continue; if(seenLines.has(k)){ issues.push('Duplicate catalogue line detected.'); break; } seenLines.add(k); }
  // Do not let an answer from a different material type leak into a selected type.
  if(materialType && materialType!=='All Materials' && r.materialType && normalizeFactText(r.materialType)!==normalizeFactText(materialType)) issues.push('Material-type mismatch.');
  if(issues.length){
    return {ok:false,issues,ledger};
  }
  r.verification=(str(r.verification)+' Smart Accuracy Gate: facts locked to supplied question; prompt leakage, duplicate lines and unsupported access points checked.').trim();
  r.accuracy={status:'verified',checks:['fact-lock','prompt-leakage','identifier-check','access-point-check','duplicate-line-check','material-type-check']};
  return {ok:true,result:r,ledger};
}
...
function normalizeResult(raw, question, section, materialType){
  ...
  const gated=smartAccuracyGate(base, question, materialType);
  if(!gated.ok){
    const fallback = localParse(question, section, materialType);
    fallback.provider = 'AACR2 Smart Accuracy Fallback';
    fallback.warning = gated.issues.join(' ');
    fallback.verification = (fallback.verification||'') + ' Primary result was rejected by the Smart Accuracy Gate; only evidence-grounded fallback data was retained.';
    fallback.accuracy={status:'fallback-after-validation',issues:gated.issues};
    return finalizeCards(fallback);
  }
  return finalizeCards(gated.result);
}
...
async function geminiRequest(question, section, materialType, useSearch=true, apiKey=""){
  ...
  const base={systemInstruction:{parts:[{text:SYSTEM_PROMPT}]},contents:[{role:"user",parts:[{text:prompt}]}],tools:useSearch?[{google_search:{}}]:[],generationConfig:{temperature:0.05,responseMimeType:"application/json"}};
...
app.post("/generate",async(req,res)=>{
  ...
  if(keys.length && !forceLocal){
    try{
      const result=await geminiRequest(q,section,materialType,true,keys[0]);
  ...
  try{
    ...
    const gated=smartAccuracyGate(fallback,q,materialType);
    if(gated.ok) return res.status(200).json(finalizeCards(gated.result));
    fallback.accuracy={status:'local-validation',issues:gated.issues};
    fallback.verification += ' Smart Accuracy Gate completed; unsupported generated facts were not promoted to the card.';
...
app.get("/health",(req,res)=>res.json({ok:true,version:VERSION,geminiConfigured:geminiKeys(req).length>0,pdf:true,unlimitedCards:true,smartAccuracyGate:true,factLock:true,duplicateGuard:true}));
...
function drawCard(doc,card,no,total){
  ...
  const rows=10, headerH=25, rowH=(H-headerH)/rows;
  ...
  for(let i=1;i<rows;i++){const y=headerH+i*rowH;doc.moveTo(0,y).lineTo(W,y).stroke();}
  ...
  const fs=7.15;
  let y=headerH+1.5;
  for(const item of prepared.slice(0,rows)){
    const text=str(item.text); if(!text){y+=rowH;continue;}
    // Traditional Type-II presentation: first line of each paragraph starts at
    // the second indention; wrapped continuation lines return to the first.
    // This keeps the card readable and prevents text from crossing the ruling.
    let x=descriptionX, width=textW;
    if(item.kind==='continuation'){x=fieldX;width=right-fieldX;}
    doc.fillColor('#000').font('Courier-Bold').fontSize(fs)
      .text(text,x,y,{width,height:rowH-2,ellipsis:false,lineBreak:false,continued:false,align:'left'});
    y+=rowH;
  }
  doc.restore();
}
