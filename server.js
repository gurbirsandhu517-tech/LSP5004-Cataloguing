...
const VERSION = "V45-AACR2-INTELLIGENT-HYBRID-CARD";
...
Operate as an INTELLIGENT HYBRID CATALOGUING ENGINE: first understand the complete question semantically, then
extract bibliographic facts, select the material treatment, construct the AACR2/ISBD-style description, validate it
against the supplied facts, and only then render the catalogue card. Never produce a pretty-looking but factually
unsupported entry. Gemini is primary when configured, while the deterministic local AACR2 engine is the guaranteed
fallback and must be capable of producing a useful entry without an API key. Google Search grounding is a verification/research layer, never the final answer itself.
...
- Treat explicit labelled fields as authoritative evidence. Treat natural-language statements as evidence only when
  their meaning is clear. Preserve supplied wording and punctuation where possible.
- Never invent a person, editor, publisher, place, date, edition, extent, dimensions, language, frequency, series,
  note, subject, identifier, UDC/classification number, call number or accession number.
- Never convert an absent fact into a plausible default merely to make the card look complete. A carrier default such
  as "1 videodisc" is allowed only when the selected material/question explicitly identifies a motion picture/video
  recording and no extent was supplied; do not invent running time or dimensions.
- Do not turn the instruction ("create a complete AACR2 entry...") into the title. The title must be grounded in the
  bibliographic part of the question.
- Use the selected material type as authoritative unless ALL MATERIALS is selected; with ALL MATERIALS, infer from the
  whole question, not from one isolated keyword.
- If Gemini proposes a title or fact that is not grounded in the question, reject that proposal and use the local
  structured parser rather than displaying hallucinated data.
- Keep one complete main catalogue card. Added entries and subject tracing stay on that card. Create a continuation
  card only for physical overflow.
- Final output must be a catalogue entry, not an explanation of the engine's reasoning.
- Prescribed punctuation and card formatting must be consistent across Book, Serial, Map, Motion Picture, Video,
  Sound, Electronic Resource and Microform treatments.
...
function localParse(question, section, materialType){
  ...
  if(inferredType==='Motion Picture' && !format && /motion picture|feature film|film/i.test(q)) physicalParts.unshift('1 videodisc');
  if(inferredType==='Video Recording' && !format && /dvd|videodisc|videotape|video recording/i.test(q)) physicalParts.unshift('1 videodisc');
  if(inferredType==='Sound Recording' && !format && /sound recording|audio recording|compact disc|audio disc/i.test(q)) physicalParts.unshift('1 sound recording');
  if(inferredType==='Microform' && !format && /microform|microfilm|microfiche/i.test(q)) physicalParts.unshift('1 microform');
  if(inferredType==='Electronic Resource' && !format && /electronic resource|website|database|online resource|computer file|https?:/i.test(q)) physicalParts.unshift('1 electronic resource');
  if(inferredType==='Map' && !format && /\bmap\b|atlas|cartographic/i.test(q)) physicalParts.unshift('1 map');
...
function intelligenceValidate(raw, question, section, materialType){
  const r = raw && typeof raw === 'object' ? JSON.parse(JSON.stringify(raw)) : {};
  const q = str(question);
  const title = str(r.titleStatement) || str(r.mainEntry) ||
    str(r.cards?.find(c=>/^MAIN ENTRY$/i.test(str(c.entryType)))?.lines?.[0]);
  if(!title || !titleIsGroundedInQuestion(title,q)) return null;

  // Conservative evidence gate. The purpose is to reject obviously unsupported
  // AI output, not to replace the AACR2 parser.
  const evidence=q.toLowerCase();
  const has=(...terms)=>terms.some(t=>evidence.includes(t));
  if(str(r.standardNumber) && /isbn/i.test(r.standardNumber) && !has('isbn')) r.standardNumber='';
  if(str(r.standardNumber) && /issn/i.test(r.standardNumber) && !has('issn')) r.standardNumber='';

  if(Array.isArray(r.cards)){
    const main=r.cards.find(c=>/^MAIN ENTRY$/i.test(str(c.entryType)))||r.cards[0];
    r.cards=main ? [{...main,entryType:'MAIN ENTRY',cardType:'Main Entry'}] : [];
  }
  return r;
}
...
Perform a two-pass task internally: (1) extract and understand only the bibliographic facts actually supplied; (2)
construct and self-check the AACR2 catalogue entry. Reject any field you cannot ground in the question. Do not invent
names, dates, publishers, places, editions, extent, notes, subjects, identifiers, UDC or call numbers. Keep the full
main entry together in one card; continuation is only for physical overflow. Return ONLY one JSON object matching the
catalogue schema requested by the system instruction. Never return the question or a prose explanation outside the
JSON object.`;
...
app.post("/generate",async(req,res)=>{
  ...
  if(keys.length && !forceLocal){
    try{
      ...
      const vetted=intelligenceValidate(result,q,section,materialType);
      if(!vetted) throw new Error("AI output failed the bibliographic evidence check.");
      const normalized=normalizeResult(vetted,q,section,materialType);
  ...
  try{
    ...
    fallback.provider=keys.length?"Hybrid local validator":"Intelligent AACR2 engine";
    fallback.warning="";
...
function drawCard(doc,card,no,total){
  ...
  for(const item of prepared.slice(0,Math.floor((H-headerH-5)/rowH))){
    ...
    else if(item.kind==='sub'){x=descriptionX;width=textW;}
...
app.get("/version",(req,res)=>res.set("Cache-Control","no-store").json({version:VERSION,cardSize:"12.5 × 7.5 cm",primary:"Intelligent Hybrid AACR2 Engine",geminiConfigured:Boolean(process.env.GEMINI_API_KEY||process.env.GOOGLE_API_KEY||process.env.GOOGLE_GEMINI_API_KEY||process.env.API_KEY),verification:"Gemini when configured + deterministic evidence-gated AACR2 fallback",materials:"All / Book / Serial Publication / Map / Motion Picture / Video Recording / Sound Recording / Electronic Resource / Microform",pageLimit:"One catalogue entry per question/card; automatic continuation cards only for physical overflow; no truncation",pdf:true,cardRules:"12.5 × 7.5 cm; Type-II paragraph start at second indention; wrapped continuation returns to first indention; horizontal/vertical ruling; one complete main card with continuation only on physical overflow"}));
